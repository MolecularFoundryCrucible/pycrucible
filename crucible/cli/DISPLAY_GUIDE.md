...
The interactive shell startup banner is a packaged text asset rather than an external user file. Render it as a consistently padded light-blue panel with one blank row above and below the logo and two light-blue columns at each side. Use `%` as a mask for solid Crucible dark-blue cells, `.` or `■` as bold white marks on dark blue, and `=`, `▄`, `█`, or `▒` as bold orange accents. Orange accents use a light-blue background in the top logo row and a dark-blue background from the second logo row onward. Show the banner only on an interactive terminal at least 36 columns wide, and render it without color when color is disabled.
