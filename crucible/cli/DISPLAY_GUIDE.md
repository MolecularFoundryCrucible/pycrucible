...
The interactive shell startup marks are packaged 16×16 pixel maps rather than external user files. Pair two source rows in each terminal row using an upper-half block, with the upper pixel as foreground and the lower pixel as background, so one terminal cell preserves approximately square source pixels. Center each mark within the current terminal width. `crucible_ascii.txt` receives one light-blue source-pixel row above and below plus two light-blue columns at each side. `crucible_ascii_full.txt` contains its own boundary and must not receive additional padding. Keep the surrounding terminal area uncolored. In each pixel map, `_` is light blue, `%` is dark blue, `=` is orange, and `.` is off-white. Show the marks only on an interactive terminal at least 20 columns wide. When color is disabled, use full and half blocks to preserve the dark-blue and orange silhouette while treating light blue and off-white as empty space.
