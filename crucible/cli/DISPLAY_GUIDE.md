...
The interactive shell startup banner is a packaged 16×16 pixel map rather than an external user file. Pair two source rows in each terminal row using an upper-half block, with the upper pixel as foreground and the lower pixel as background, so one terminal cell preserves approximately square source pixels. Center the resulting 20-column panel within the current terminal width. The panel has one light-blue source-pixel row above and below the logo and two light-blue columns at each side. Keep the surrounding terminal area uncolored. In the pixel map, `_` is light blue, `%` is dark blue, `=` is orange, and `.` is off-white. Show the banner only on an interactive terminal at least 20 columns wide. When color is disabled, use full and half blocks to preserve the dark-blue and orange silhouette while treating light blue and off-white as empty space.
