...
The interactive shell startup banner is a packaged text asset rather than an external user file. Render it as a consistently padded light-blue panel with one blank row above and below the logo and two light-blue columns at each side. Use `%` as a mask for solid Crucible dark-blue cells and `.` as bold white marks on dark blue. Render `=` in bold Crucible orange on light blue in the top logo row and on dark blue from the second logo row onward. Show it only on an interactive terminal at least 36 columns wide, and render it without color when color is disabled.
