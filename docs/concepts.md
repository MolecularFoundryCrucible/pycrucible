...
## Users
Users can be people or service accounts with access to Crucible.
Users can be members of access groups and projects.
...
Client parameters ending in `_mfid` require a canonical MFID.
Parameters ending in `_ref` accept more than one documented identifier format and dispatch accordingly.
Canonical user identifiers can be ORCIDs or MFIDs, and an MFID does not by itself identify a service account, so user APIs use `user_unique_id` when either shape is accepted.
